...
// ⚠️ ก็อปมาจาก public/js/modes/INF/inf-mode.js (เฉพาะส่วนตรรกะล้วนๆ ไม่มี DOM)
// เพราะไฟล์ต้นทางผสม DOM code (document.getElementById ฯลฯ) ปนอยู่ทั้งไฟล์ รันบน Node ไม่ได้
// ถ้าจะปรับสมดุล/สเกลของ INF (STAGE1_FIXED_STATS, STAGE1_BASE_SKILLS, สูตรสเกล) ต้องแก้ทั้ง 2 ที่
...
function generateInfStage(stageNumber, teamSize = 4) {
  ...
  for (let i = 0; i < teamSize; i++) {
    ...
    const stagePercent = (stageNumber / 50) * 100;
    ...
    enemies.push({ name: `${cls} Lv${stageNumber}`, class: cls, hp, atk, def, skill });
  }
  return enemies;
}

...
// feature: a blacklisted roll simply never enters the deck (no card, no refund) —
// this used to be entirely unenforced here (every rolled card was pushed into deck
// no matter what), so ticking a character off in the UI did nothing.
// ---------------------------------------------------------------------------
  ...
  try {
    ...
    for (let i = 0; i < times; i++) {
      ...
      if (blacklistSet.has(card.name)) {
        results.push({ ...card, _blacklisted: true });
      } else {
        results.push(card);
        kept.push(card);
      }

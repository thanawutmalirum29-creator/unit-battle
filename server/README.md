...
## ล็อกอินรายวัน + ภารกิจรายวัน (ใหม่)

**เวลารีเซ็ต — สำคัญ อ่านตรงนี้ก่อน:**
- **"1 วันเกม" ไม่ใช่เที่ยงคืน และไม่ใช่ 24 ชม.นับจากตอนล็อกอิน** แต่คือช่วงเวลาที่รีเซ็ตทุก
  **ตี 4 (04:00 น.) ตามเวลาประเทศไทย (UTC+7)** ทุกวัน — เช่น ถ้าเล่นตอนตี 3:59 น. กับตอนตี 4:01 น.
  ของวันเดียวกันตามนาฬิกา จะนับเป็นคนละ "วันเกม" กัน
- **"1 สัปดาห์เกม"** คือช่วงที่รีเซ็ตทุก **วันอาทิตย์ เวลาตี 4 (04:00 น.) ตามเวลาประเทศไทย** — ใช้ตรรกะเดียวกัน
  กับระบบกิลด์ที่มีอยู่แล้ว (guild boss / guild shop รายสัปดาห์)
- ประเทศไทยไม่มี Daylight Saving Time เลยใช้ offset คงที่ UTC+7 ได้ตลอดโดยไม่ต้องกังวลเรื่องเปลี่ยนเวลา

ตรรกะรีเซ็ตนี้คำนวณจาก `currentDayIndex()` ใน `game-data/daily-data.js` (เลขจำนวนเต็มที่เพิ่มขึ้นทีละ 1
ทุกครั้งที่ข้ามตี 4 เวลาไทย) — เป็นสูตรเดียวกับ `currentGameDayStart()` / `currentGuildCycle()` ใน
`game-data/guild-data.js` (คนละไฟล์ แต่ตั้งใจให้สูตรตรงกันเป๊ะ เพื่อให้ "วันเกม/สัปดาห์เกม" หมายถึงช่วงเวลา
เดียวกันไม่ว่าจะมองจากระบบไหนในเกม) ไม่มี cron job แยก — ทุกอย่างรีเซ็ตแบบ "lazy" คือแค่คำนวณ
`currentDayIndex()` ใหม่ทุกครั้งที่มีคำขอเข้ามา ถ้าเลขวันเปลี่ยนไปจากที่บันทึกไว้ก็ถือว่าเป็นวันใหม่ทันที

**ล็อกอินรายวัน:** กดรับได้วันละ 1 ครั้ง ต่อ "วันเกม" (ดูนิยามด้านบน) ให้เงิน/ไอเทมตามสตรีคต่อเนื่อง
7 วันแล้ววนซ้ำ (วันที่ 7 ได้รางวัลก้อนใหญ่สุด) ถ้าเว้นข้าม 1 วันเกมไป สตรีคจะรีเซ็ตกลับไปวันที่ 1 —
ตารางรางวัลแก้ได้ที่ `LOGIN_REWARD_CYCLE` ใน `game-data/daily-data.js`

**ภารกิจรายวัน:** ชุดภารกิจคงที่ 5 ข้อ (ชนะการต่อสู้, สุ่มกาชา, ซื้อร้านค้า, โจมตี PvP ฯลฯ) ความคืบหน้า
รีเซ็ตเป็น 0 ทุกครั้งที่ขึ้น "วันเกม" ใหม่ (ไม่ต้องมี cron ลบข้อมูล เพราะคีย์บันทึกผูกกับ `day_index` อยู่แล้ว
วันเกมใหม่ = แถวใหม่โดยอัตโนมัติ) ทำครบทุกข้อในวันเดียวกันจะปลดล็อกโบนัสพิเศษเพิ่มอีกก้อนหนึ่ง —
แก้/เพิ่มภารกิจได้ที่ `DAILY_MISSIONS` ใน `game-data/daily-data.js`

**ไฟล์ที่เกี่ยวข้อง:** `game-data/daily-data.js` (ตัวเลข/สูตรเวลา), `db/dailyMissions.js` (บันทึกความคืบหน้า
จาก route อื่น), `routes/daily.js` (API), `public/js/core/daily-fab.js` (ลูกบอลลอย 🎁 + ป๊อปอัป UI),
ตาราง `daily_login_state` / `daily_mission_progress` ใน `schema.sql`
